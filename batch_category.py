...
INPUT_ROOT = Path(r"e:\OpenStyle\UI\input\category")
OUTPUT_ROOT = Path(r"e:\OpenStyle\UI\output\category")
CACHE_PATH = Path(r"e:\OpenStyle\.openstyle-category-cache.json")
...
NOISE_WORDS = frozenset({
    ...
    "html", "html5", "css", "css3", "bootstrap", "bootstrap4", "responsive", "static",
...
CATEGORY_KEYWORDS = {
    ...
    "children": "kids", "toy": "toy", "advertising": "advertising",
...
def find_index_files(template_dir: Path) -> list[Path]:
    results: list[Path] = []
    for pattern in ("index.html", "index.htm", "Index.html", "INDEX.HTML"):
        results.extend(template_dir.rglob(pattern))
    ...
    for p in sorted(results):
        ...
        if r in seen:
            continue
        try:
            rel = r.relative_to(template_dir.resolve())
        except ValueError:
            continue
        if len(rel.parts) > 6:
            continue
...
def main() -> None:
    ...
    for cat_dir in categories:
        ...
        cat_output = OUTPUT_ROOT / cat_name
        cat_output.mkdir(parents=True, exist_ok=True)

        used_slugs: set[str] = {p.name for p in cat_output.iterdir() if p.is_dir()}
        ...
        print(f"\n=== Category: {cat_name} ({len(bundles)} bundles) ===")
        ...
        for bundle in bundles:
            ...
            slug = make_output_slug(english_title, seed=f"{cat_name}/{bundle.name}")
            ...
            for idx, index_file in enumerate(index_files):
                ...
                if len(index_files) == 1:
                    target = cat_output / slug
                else:
                    target = cat_output / f"{slug}-v{idx + 1}"
